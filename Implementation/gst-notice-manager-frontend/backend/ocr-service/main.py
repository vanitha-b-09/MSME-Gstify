...
def parse_text(text):
    ...
    # Invoice number (common variants)
    invoice_no = re.search(r"(Invoice\s*(No|Number)\s*[:#\s-]*)\s*([A-Za-z0-9\/-]+)", text, re.IGNORECASE)

    # Amount (try a few patterns; OCR is messy)
    amount = re.search(r"(Grand\s*Total|Total\s*Amount|Invoice\s*Total)\D*([0-9][\d,]*\.?\d{0,2})", text, re.IGNORECASE)

    sections = re.findall(r"(?:Section|Sec\.?)\s*(\d+)", text, re.IGNORECASE)
    ...
    return {
        ...
        "invoice_number": invoice_no.group(3) if invoice_no else None,
