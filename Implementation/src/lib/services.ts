...
export const casesService = {
  ...
  list: async (user?: Partial<User> | null) => {
    if (USE_MOCKS) {
      return await mock.listCases(user);
    }
    const res = await API.get("/cases");

    return res.data.map((c: { id: string; client_name: string; client_gstin: string; status?: string; created_at?: string }) => ({
      id: c.id,
  ...
  reconcileGstr: async (caseId: string) => {
    if (USE_MOCKS) {
      return {
    ...
    const res = await API.post(`/reconcile/gstr/${caseId}`);
    return res.data as {
      message: string;
      data: {
        status?: string;
...
export const caseFilesService = {
  ...
  reprocess: async (fileId: string) => {
    if (USE_MOCKS) return { message: "mock reprocess" };
    const res = await API.post(`/upload/file/${fileId}/reprocess`);
    return res.data as { message: string; parsed_data?: unknown };
  },

  fileContentUrl: (fileId: string) => {
    const base = (API.defaults.baseURL || "").replace(/\/$/, "");
    return `${base}/upload/file/${encodeURIComponent(fileId)}/content`;
  },
};
